"""Download/export SF1 fundamentals and write the filtered fundamentals dataset."""
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download/build SF1 fundamentals")
    parser.add_argument(
        "--input-file",
        action="append",
        default=None,
    ...
    parser.add_argument(
        "--bulk-download",
        action="store_true",
        help="Request a Nasdaq Data Link SF1 table export and download it before converting. This is the default when no --input-file is supplied.",
    )
    parser.add_argument("--reference-dir", default=None, help=f"Directory containing tickers.parquet. Default: {DEFAULT_REFERENCE_DIR}")
    parser.add_argument("--raw-export-dir", default=None, help=f"Directory for downloaded SF1 export files. Default: {DEFAULT_RAW_EXPORT_DIR}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        output_path = build_sf1_from_files(
            input_files,
            reference_dir=args.reference_dir,
            output_dir=args.output_dir,
            chunk_rows=args.chunk_rows,
        )
        logger.info("Built SF1 fundamentals output=%s total_time=%.3fs", output_path, time.perf_counter() - started)
        return 0
    except Exception:
        logger.exception("Building SF1 fundamentals failed")
        return 1
